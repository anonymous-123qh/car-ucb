...
DEADLINE_TAU = 100.0      # ms
SIMULATION_STEPS = 5000   # number of decision

# Experiment Settings
NUM_TRIALS = 20           #run 20 independent trials
...
class TraceEnvironment:
    ...
    def dynamic_trace(self):
        ...
        T = SIMULATION_STEPS + 100
        PHASE_LEN = 500
        NUM_GOOD = max(1, self.num_paths // 3)
...
def run_single_trial(trial_seed, trial_id = None, step_writer = None):
    ...
    env = TraceEnvironment(TOTAL_PATHS, mode="dynamic")
    ...
    metrics = {name: {"success": 0, "cost": 0.0, "switches": 0, "last_act": None, "total_size": 0, "dec_times": []} for
               name in agents}
    # Track latencies only for the final aggregation if needed, or summary stats
    lat_sums = {name: 0.0 for name in agents}
    lat_counts = {name: 0 for name in agents}
    ...
    for t in range(SIMULATION_STEPS):
        ...
        for name, agent in agents.items():
            ...
            if latencies:
                lat_sums[name] += min(latencies.values())
                lat_counts[name] += 1
            ...
            if step_writer is not None:
                ...
                latencies_json = json.dumps(latencies_clean)
    ...
    for name, data in metrics.items():
        summary[name] = {
            ...
            "avg_latency": lat_sums[name] / lat_counts[name] if lat_counts[name] > 0 else 0,
            "avg_dec_time": sum(data["dec_times"]) / len(data["dec_times"])
        }
    return summary


def run_experiment_suite():
    print(f"Starting Experiment Suite: {NUM_TRIALS} Trials, {SIMULATION_STEPS} Steps each.")
    os.makedirs("data_output", exist_ok=True)
    algo_names =["CAR-UCB1", "CAR-UCB2", "CAR-UCB3", "FT-Flood"]
    # Store all results
    all_results = {
        name: {
            "success_rate": [],
    ...
    summary_path = f"data_output/multiple_robot_trial_summary_{NUM_INTERFACES}_{NUM_SERVERS}.csv"
    with open(raw_path, "w", newline="") as f_raw, \
        ...
        for i in range(NUM_TRIALS):
            trial_seed = 2024 + i
            print(f"  -> Running Trial {i + 1}/{NUM_TRIALS} (seed={trial_seed})...", end="\r")

            trial_res = run_single_trial(trial_seed=trial_seed, trial_id=i,
                                         step_writer=raw_writer)  # Use deterministic seeds 2024, 2025...
    ...
    print(f"{'Algorithm':<12} | {'Success Rate':<20} | {'Avg Cost':<15} | {'Avg Latency':<18} | {'Switches':<15}")
    print("-" * 90)
    ...
    for name, metrics in all_results.items():
        ...
        print(f"{name:<12} | {sr_mean * 100:6.2f}% +/- {sr_std * 100:4.2f}% | "
